...
async def handle_gduser(bot: Bot, event: Event, arg: Message = CommandArg()) -> None:
    ...
    user_classic_nondemon = (
        ...
        if user.classic_levels and len(user.classic_levels) >= 8
        else ""
    )
    user_plat_nondemon = (
        ...
        if user.platformer_levels and len(user.platformer_levels) >= 6
        else ""
    )
    user_demon = (
        ...
        if user.demons_breakdown and len(user.demons_breakdown) >= 12
        else ""
    )
