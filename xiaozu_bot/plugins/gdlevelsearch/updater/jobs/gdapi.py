        ...
        while i < len(parts) - 1:
            ...
            if key in cls.FIELD_MAP:
                ...
                if typ is int:
                    ...
                    value_str = value_str.replace("-","+").replace("_","/") #robtop!!!!!!!!!
                    if len(value_str) % 4 != 0:
                        value_str += "=" * (4-(len(value_str)%4))
                    try:
                        processed_value = base64.b64decode(value_str).decode("utf-8")
                    except Exception as e:  # noqa: BLE001
                        logger.error(f"base64解码错误：{e.__str__()} for {value_str}")
                        processed_value = value_str
...
def search_levels(  # noqa: C901, PLR0912, PLR0913, PLR0915
    ...
    **kwargs: Any,
    ...
    if len(parts) < 4:  # noqa: PLR2004
        logger.error(f"Cant get levels from response: {text}")
        raise ValueError(f"Cant get levels from response: {text}")  # noqa: TRY003
