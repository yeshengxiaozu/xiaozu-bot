...
setmode = on_command('setmode', permission=GROUP, priority=1, block=True, rule=whitelist_rule)
# 地下酒馆 - “游戏”判定
bet = on_command('betgame', permission=GROUP, priority=100, block=True, rule=whitelist_rule)

@setmode.handle()
async def handle_function(bot: Bot, event: GroupMessageEvent, arg: Message = CommandArg()):
    args = str(arg).lower().split(' ')
    if len(args) != 1 or not str.isdigit(args[0]):
        await setmode.finish("请输入一个整数！")
    id = int(args[0])
    if id<0 or id>2:
        await setmode.finish("目前只接受0（普通模式），1（身份模式），2（膀胱模式）这几个值作为参数哦~")
    r.hset("game_mode",str(event.user_id),str(id))
    await setmode.finish("已将你的游戏模式设置为" + ["普通模式","身份模式","膀胱模式"][id])
...
async def bet_handle(bot: Bot, event: GroupMessageEvent, arg: Message = CommandArg()):
    ...
    if True:
        ...
        if len(datas.demon_data[group_id]['pl']) != 1:
            ...
            identity_status_list = [int(r.hget("game_mode",player_id)) for player_id in player_ids]
            ...
            msg += "\n\n"
            msg += item_msg
            msg += f"\n- 总弹数{str(len(datas.demon_data[group_id]['clip']))}，实弹数{str(datas.demon_data[group_id]['clip'].count(1))}\n"
...
def death_mode(identity_found, group_id):
    ...
    if identity_found in turn_limit and datas.demon_data[group_id]['game_turn'] > turn_limit[identity_found]:
        ...
        if identity_found in [1,2] and datas.demon_data[group_id]["hp_max"] > 1:
            ...
            for i in range(len(datas.demon_data[group_id]["hp"])):
                datas.demon_data[group_id]["hp"][i] = min(datas.demon_data[group_id]["hp"][i], datas.demon_data[group_id]["hp_max"])

        # 额外扣除 1 点道具上限，并随机删除 1-2 个道具
        if identity_found in [1,2]:
            if datas.demon_data[group_id]["item_max"] > 6:
                datas.demon_data[group_id]["item_max"] -= 1  # 扣 1 点道具上限（最低仍为 6）
            ...
            # 计算可删除的道具数量
            remove_count0 = min(remove_random, len(datas.demon_data[group_id]['item_0'])) if datas.demon_data[group_id]['item_0'] else 0
            remove_count1 = min(remove_random, len(datas.demon_data[group_id]['item_1'])) if datas.demon_data[group_id]['item_1'] else 0

            # 随机选择要删除的道具
            removed_items_0 = random.sample(datas.demon_data[group_id]['item_0'], remove_count0) if remove_count0 else []
            removed_items_1 = random.sample(datas.demon_data[group_id]['item_1'], remove_count1) if remove_count1 else []
...
def refersh_item(identity_found, group_id):
    ...
    for i in range(random.randint(lower, upper)):
        datas.demon_data[group_id]['item_0'].append(get_random_item(identity_found, len(item_dic) - idt_len, player0))
        datas.demon_data[group_id]['item_1'].append(get_random_item(identity_found, len(item_dic) - idt_len, player1))
    # 检查并限制道具数量上限为max
    datas.demon_data[group_id]['item_0'] = datas.demon_data[group_id]['item_0'][:item_max]  # 截取前max个道具
    datas.demon_data[group_id]['item_1'] = datas.demon_data[group_id]['item_1'][:item_max]  # 截取前max个道具
    # 生成道具信息
    item_0 = ", ".join(item_dic.get(i, "未知道具") for i in datas.demon_data[group_id]['item_0'])
    item_1 = ", ".join(item_dic.get(i, "未知道具") for i in datas.demon_data[group_id]['item_1'])
    # 获取玩家道具信息
...
async def shoot(stp, group_id, message,args):
    ...
    if datas.demon_data[group_id]['hcf'] < 0 and stp != 0:
        ...
        out_pl = datas.demon_data[group_id]['pl'][datas.demon_data[group_id]['turn']-1]
    ...
    if datas.demon_data[group_id]['hp'][0] <= 0: 
        ...
        pid = datas.demon_data[group_id]['pl'][datas.demon_data[group_id]['turn']]
        msg += '- 本局总弹数为'+str(len(datas.demon_data[group_id]['clip']))+'，实弹数为'+str(datas.demon_data[group_id]['clip'].count(1))+"\n" + "- 当前是"+ MessageSegment.at(pid) + "的回合"
...
async def prop_demon_handle(bot: Bot, event: GroupMessageEvent, arg: Message = CommandArg()):
    ...
    hp_max = datas.demon_data.get(group_id, {}).get('hp_max')
    item_max = datas.demon_data.get(group_id, {}).get('item_max')
    msg = MessageSegment.at(str(datas.demon_data[group_id]["pl"][player_turn])) + f"使用了道具：{item_name}\n\n"
    ...
    if item_name == "桃":
        ...
        if datas.demon_data[group_id]["hp"][player_turn] >= hp_max:
        ...
        if datas.demon_data[group_id]["hp"][player_turn] >= hp_max:
            datas.demon_data[group_id]["hp"][player_turn] = hp_max
        datas.demon_data[group_id]['turn'] = (player_turn + 1) % len(datas.demon_data[group_id]['pl'])
        ...
        if not datas.demon_data[group_id]['clip'] or all(b == 0 for b in datas.demon_data[group_id]['clip']):
            datas.demon_data[group_id]['clip'] = load(identity_found)
            msg += "- 子弹已耗尽，重新装填！\n"
            msg += "\n"
            # 游戏轮数+1
            ...
            # 增加弹数消息，优化排版
            msg += '\n- 本局总弹数为'+str(len(datas.demon_data[group_id]['clip']))+'，实弹数为'+str(datas.demon_data[group_id]['clip'].count(1))
        ...
        msg += f"对方的体力为 {datas.demon_data[group_id]['hp'][opponent_turn]}/{hp_max}\n"
        ...
        if datas.demon_data[group_id]["hp_max"] <= 1:
            ...
            hp_max -= 1
            item_max += 1
            datas.demon_data[group_id]["hp_max"] = max(1, datas.demon_data[group_id]["hp_max"])  # 血量上限保护锁
        ...
        if current_hp_max <= 1 or current_hp <= 1 or (current_hp_max == 2 and current_hp == 2):
            ...
            datas.demon_data[group_id]["hp_max"] -= 1
            datas.demon_data[group_id]["hp_max"] = max(1, datas.demon_data[group_id]["hp_max"])  # 确保体力上限不会降到 0，虽然前面有判断了
    ...
    next_player_id = str(datas.demon_data[group_id]["pl"][next_player_turn])  # 下一位玩家的 ID
...
async def check_handle(event: GroupMessageEvent):
    ...
    msg += MessageSegment.at(player1) + f"\nhp：{hp1}/{hp_max}\n" + f"道具({len(items_1)}/{item_max})：" +f"\n{item_1}\n\n"
    msg += f"- 总弹数{str(len(datas.demon_data[group_id]['clip']))}，实弹数{str(datas.demon_data[group_id]['clip'].count(1))}\n"
    pid = datas.demon_data[group_id]['pl'][datas.demon_data[group_id]['turn']]
    msg += "- 当前是"+ MessageSegment.at(pid) + "的回合"
    ...
    if elapsed > turn_time:  # 全局变量，设定时间（秒）
        ...
        if datas.demon_data[group_id]['start']:
            ...
            if len(datas.demon_data[group_id]['pl']) == 1:
                ...
                await bot.send_group_msg(
                    group_id=group_id,
                    message=f"由于长时间无第二人进入轮盘，现已重置游戏。"
                )
                return True
    return False
