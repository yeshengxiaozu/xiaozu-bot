...
async def handle_function(bot: Bot, event: Union[GroupMessageEvent,PrivateMessageEvent], arg: Message = CommandArg()):
    ...
    try:
        file_path = await asyncio.to_thread(
            sync_generate_audio,
            text, instruct, output_dir
        )
    except Exception as e:
        await say.finish(f"生成音频失败: {e}")
    if isinstance(event,GroupMessageEvent):
        requests.post('http://localhost:3000/send_group_msg', json = json_group_audio(event.group_id,file_path))
    else:
        requests.post('http://localhost:3000/send_private_msg', json = json_private_audio(event.user_id,file_path))
    os.remove(file_path)
    await say.finish()
...
async def handle_function(bot:Bot, event: Union[GroupMessageEvent,PrivateMessageEvent], arg: Message = CommandArg()):
    ...
    if len(text) == 0:
        await say_instructed.finish("两个参数，第一个参数是指令参数，第二个参数是文本内容哦！")
    if len(text) > 1000:
        await bot.call_api("set_msg_emoji_like",message_id=event.message_id, emoji_id = "424")
