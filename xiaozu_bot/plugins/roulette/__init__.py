...
async def handle_function(event: GroupMessageEvent):
    ...
    await roulette.finish("你来到了那个轮盘原本所在的位置，只看到一块写着打烊的招牌。这个轮盘不会再转动了。")
    return
    id = event.user_id
...
async def handle_function():
    await get_pool.finish("你来到了蓝莓奖池旁。里面空空如也，周围的护栏呈现出风化的痕迹。看来没有人会再关心里面有多少蓝莓了。")
...
async def handle_function(event: GroupMessageEvent, arg: Message = CommandArg()):
    ...
    await donate.finish("你来到了蓝莓奖池旁。里面空空如也，周围的护栏呈现出风化的痕迹。你扔了一个或者一些不存在的蓝莓进去。那不重要。")
    return
...
async def handle_function():
    ...
    await roulette_count.finish(f"池子旁边的一块石头上刻着{t}条密密麻麻的线。什么时候这些线也会被时间磨平呢？")
