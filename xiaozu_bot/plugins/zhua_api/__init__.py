...
class berry_manager:
    ...
    def is_berrybot(event: GroupMessageEvent) -> bool:
        return (
            event.get_user_id() == "3948837959"
            or event.get_user_id() == "3928744142"
            or event.get_user_id() == "914578128"
        )
...
async def handle_function(event: GroupMessageEvent, arg: Message = CommandArg()):
    await buy.finish(
        "你带着一袋子草莓来到了蓝莓银行的门口，但是那块写着银行的牌子下面没有人也没有伯特。看样子你在这做不了任何事了。"
    )
    return
    ...
    if len(args) != 1 or not args[0].isdecimal() or int(args[0]) <= 0:
...
async def handle_function(event: GroupMessageEvent, arg: Message = CommandArg()):
    await sell.finish(
        "那块写着银行的牌子下面没有人也没有伯特，你也明知道你的账户已经在那天被清空了。你想从这里得到什么？"
    )
    return
    if event.group_id != outer_group_id:
...
async def handle_function(event: GroupMessageEvent):
    await check.finish(
        "原本挂着账单的地方现在只有一张白纸。不需要任何人进行任何查询，你知道你的账户——如果它还存在的话——空空如也。"
    )
    return
    if event.group_id != outer_group_id:
...
async def handle_function(event: MessageEvent, arg: Message = CommandArg()):
    ...
    if t > 0:
        ...
        await msg.send_at(
            id, "已经没有蓝莓相关功能了……但至少此时此刻，小小卒仍然可以禁用你的guess。"
        )
