...
def banned(event: Event):
    return is_group_event(event) and get_group_id(event) == "569801410"


def notbanned(event: Event):
...
async def handle_function(
    ...
    if len(args) == 0:
        ...
        await react(bot, event, "128560", message_id=extract_sent_message_id(tmsg))
        await ultra.finish()
    elif len(args) < 2:
        await react(bot, event, "424")
        await ultra.finish()
...
async def handle_function(bot: Bot, event: Event):
    ...
    if pick == 1:
        ...
        await react(bot, event, "424", message_id=extract_sent_message_id(tmsg))
    elif pick == 2:
        tmsg = await nsdd.send(
...
async def handle_function(
    ...
    await react(bot, event, "10068", message_id=extract_sent_message_id(tmsg))
    await jwz.finish()
...
async def handle_function(
    ...
    await react(bot, event, "144", message_id=extract_sent_message_id(tmsg))
...
@test.handle()
async def handle_function(event: Event, arg: Message = CommandArg()):
